...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
	...
	func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
		...
		currentSymbol = currencySymbol[row]
...
//    
    ...
    func updateBitcoinRate(json : JSON) {
		...
		if let tempResult = json["ask"].double{
			let updatedLabel = currentSymbol + String(tempResult)
			bitcoinPriceLabel.text = String(updatedLabel)
			print(tempResult)
		}
		else{
			bitcoinPriceLabel.text = "Rates"
		}
